...
assert(not shared.VapeExecuted, "FapeClient Already Injected")
shared.VapeExecuted = true

for i,v in pairs({baseDirectory:gsub("/", ""), "FapeClient", "FapeClient/Libraries", "FapeClient/CustomModules", "FapeClient/Profiles", baseDirectory.."Profiles", "FapeClient/assets"}) do 
	if not isfolder(v) then makefolder(v) end
end
task.spawn(function()
	local success, assetver = pcall(function() return vapeGithubRequest("assetsversion.txt") end)
	if not isfile("FapeClient/assetsversion.txt") then writefile("FapeClient/assetsversion.txt", "0") end
	if success and assetver > readfile("FapeClient/assetsversion.txt") then
		redownloadedAssets = true
		if isfolder("FapeClient/assets") and not shared.VapeDeveloper then
			if delfolder then
				delfolder("FapeClient/assets")
				makefolder("FapeClient/assets")
			end
		end
		writefile("FapeClient/assetsversion.txt", assetver)
	end
end)
if not isfile("FapeClient/CustomModules/cachechecked.txt") then
	...
	if isNotCached and not shared.VapeDeveloper then
		displayErrorPopup("FapeClient has detected uncached files, If you have CustomModules click no, else click yes.", {No = function() end, Yes = function()
			for i,v in pairs({"FapeClient/Universal.lua", "FapeClient/MainScript.lua", "FapeClient/GuiLibrary.lua"}) do 
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					delfile(v)
				end 
			...
			for i,v in pairs(listfiles("FapeClient/CustomModules")) do 
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					local last = v:split('\\')
					last = last[#last]
					local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/ImNicknamez/FapeClient/"..readfile("FapeClient/commithash.txt").."/CustomModules/"..last) end)
					if suc and publicrepo and publicrepo ~= "404: Not Found" then
						writefile("FapeClient/CustomModules/"..last, "--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.\n"..publicrepo)
					end
				end 
			end
		end})
	end
	writefile("FapeClient/CustomModules/cachechecked.txt", "verified")
end
...
task.spawn(function()
	...
	image.Image = downloadVapeAsset("FapeClient/assets/CombatIcon.png")
	...
	task.spawn(function()
		task.wait(15)
		if image and image.ContentImageSize == Vector2.zero and (not errorPopupShown) and (not redownloadedAssets) and (not isfile("FapeClient/assets/check3.txt")) then 
            errorPopupShown = true
            displayErrorPopup("Assets failed to load, Try another executor (executor : "..(identifyexecutor and identifyexecutor() or "Unknown")..")", {OK = function()
                writefile("FapeClient/assets/check3.txt", "")
            end})
        end
	end)
end)
...
local Combat = GuiLibrary.CreateWindow({
	Name = "Combat", 
	Icon = "FapeClient/assets/CombatIcon.png", 
	IconSize = 15
})
local Blatant = GuiLibrary.CreateWindow({
	Name = "Blatant", 
	Icon = "FapeClient/assets/BlatantIcon.png", 
	IconSize = 16
})
local Render = GuiLibrary.CreateWindow({
	Name = "Render", 
	Icon = "FapeClient/assets/RenderIcon.png", 
	IconSize = 17
})
local Utility = GuiLibrary.CreateWindow({
	Name = "Utility", 
	Icon = "FapeClient/assets/UtilityIcon.png", 
	IconSize = 17
})
local World = GuiLibrary.CreateWindow({
	Name = "World", 
	Icon = "FapeClient/assets/WorldIcon.png", 
	IconSize = 16
})
local Friends = GuiLibrary.CreateWindow2({
	Name = "Friends", 
	Icon = "FapeClient/assets/FriendsIcon.png", 
	IconSize = 17
})
...
local Profiles = GuiLibrary.CreateWindow2({
	Name = "Profiles", 
	Icon = "FapeClient/assets/ProfilesIcon.png", 
	IconSize = 19
})
GUI.CreateDivider()
GUI.CreateButton({
...
OnlineProfilesButtonImage.Image = downloadVapeAsset("FapeClient/assets/OnlineProfilesButton.png")
...
OnlineProfilesExitButton.Image = downloadVapeAsset("FapeClient/assets/ExitIcon1.png")
...
OnlineProfilesFrameShadow.Image = downloadVapeAsset("FapeClient/assets/WindowBlur.png")
...
OnlineProfilesFrameIcon.Image = downloadVapeAsset("FapeClient/assets/ProfilesIcon.png")
...
OnlineProfilesButton.MouseButton1Click:Connect(function()
	...
	if not profilesLoaded then
        ...
        local success, result = pcall(function()
            return game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/ImNicknamez/FapeClient/main/Profiles/"..saveplaceid.."/profilelist.txt", true))
        end)
		for i,v in pairs(success and result or {}) do 
			onlineprofiles[i] = v
		end
		for i2,v2 in pairs(onlineprofiles) do
			local profileurl = "https://raw.githubusercontent.com/ImNicknamez/FapeClient/main/Profiles/"..saveplaceid.."/"..v2.OnlineProfileName
...
local TextGUI = GuiLibrary.CreateCustomWindow({
	Name = "Text GUI", 
	Icon = "FapeClient/assets/TextGUIIcon1.png", 
	IconSize = 21
})
local TextGUICircleObject = {CircleList = {}}
GUI.CreateCustomToggle({
	Name = "Text GUI", 
	Icon = "FapeClient/assets/TextGUIIcon3.png",
...
VapeLogo.Image = downloadVapeAsset("FapeClient/assets/VapeLogo3.png")
...
VapeLogoV4.Image = downloadVapeAsset("FapeClient/assets/VapeLogo4.png")
...
TextGUIMode = TextGUI.CreateDropdown({
	...
	Function = function(val)
		...
		if val == "Drawing" then
			...
			VapeLogoDrawing.Data = readfile("FapeClient/assets/VapeLogo3.png")
			...
			VapeLogoV4Drawing.Data = readfile("FapeClient/assets/VapeLogo4.png")
			...
			VapeLogoShadowDrawing.Data = readfile("FapeClient/assets/VapeLogo3.png")
			...
			VapeLogo4Drawing.Data = readfile("FapeClient/assets/VapeLogo4.png")
...
local TargetInfo = GuiLibrary.CreateCustomWindow({
	Name = "Target Info",
	Icon = "FapeClient/assets/TargetInfoIcon1.png",
	IconSize = 16
})
...
TargetInfoHealthBackgroundShadow.Image = downloadVapeAsset("FapeClient/assets/WindowBlur.png")
...
GUI.CreateCustomToggle({
	Name = "Target Info", 
	Icon = "FapeClient/assets/TargetInfoIcon2.png", 
    ...
    if (not teleportedServers) and (not shared.VapeIndependent) then
		...
		local teleportScript = [[
			...
			if shared.VapeDeveloper then 
				loadstring(readfile("FapeClient/NewMainScript.lua"))() 
			else 
				loadstring(game:HttpGet("https://raw.githubusercontent.com/ImNicknamez/FapeClient/"..readfile("FapeClient/commithash.txt").."/NewMainScript.lua", true))() 
			end
		]]
		if shared.VapeDeveloper then
...
local function loadVape()
	if not shared.VapeIndependent then
		...
		if isfile("FapeClient/CustomModules/"..game.PlaceId..".lua") then
			loadstring(readfile("FapeClient/CustomModules/"..game.PlaceId..".lua"))()
		else
			if not shared.VapeDeveloper then
				local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/ImNicknamez/FapeClient/"..readfile("FapeClient/commithash.txt").."/CustomModules/"..game.PlaceId..".lua") end)
				if suc and publicrepo and publicrepo ~= "404: Not Found" then
					writefile("FapeClient/CustomModules/"..game.PlaceId..".lua", "--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.\n"..publicrepo)
					loadstring(readfile("FapeClient/CustomModules/"..game.PlaceId..".lua"))()
				end
			end
		end
		--if shared.VapePrivate then
		if isfile("FapePrivate/CustomModules/"..game.PlaceId..".lua") then
			loadstring(readfile("FapePrivate/CustomModules/"..game.PlaceId..".lua"))()
		end	
		--end
	else
	...
	if not shared.VapeSwitchServers then
		if BlatantModeToggle.Enabled then
			pcall(function()
				local frame = GuiLibrary.CreateNotification("Blatant Enabled", "FapeClient is now in Blatant Mode.", 5.5, "assets/WarningNotification.png")
