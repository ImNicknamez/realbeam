	...
	if WhitelistFunctions:CheckPlayerType(plr) ~= "DEFAULT" or WhitelistFunctions.WhitelistTable.chattags[WhitelistFunctions:Hash(plr.Name..plr.UserId)] then
		...
		if playerlist then
			pcall(function()
				...
				if targetedplr then 
					targetedplr.ChildrenFrame.NameFrame.BGFrame.OverlayFrame.PlayerIcon.Image = getcustomassetfunc("FapeClient/assets/VapeIcon.png")
				end
			end)
		end
...
task.spawn(function()
	local function createannouncement(announcetab)
		...
		vapeiconicon.Image = getsynasset("FapeClient/assets/VapeIcon.png")
	...
	task.spawn(function()
		pcall(function()
			...
			repeat
				...
				local newdata = game:HttpGet("https://raw.githubusercontent.com/ImNicknamez/FapeClient/"..commit.."/CustomModules/bedwarsdata", true)
